...
def list_students(
    ...
    # Se há filtro por tags, fazer join com projetos e tags
    if tags:
        query = query.join(Project).join(Project.tags).filter(Tags.id.in_(tags))
    
    # Se há termo de busca, filtrar por nome do estudante ou título do projeto
    if search:
        if tags:
            # Se já tem join com Project, usar o mesmo
            query = query.filter(
                (Student.name.ilike(f"%{search}%")) | 
                (Project.title.ilike(f"%{search}%"))
            )
        else:
            # Se não tem join com Project, fazer join apenas para busca
